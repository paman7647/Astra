...
JS_ENGINE_SOURCE = "\n".join([
 ...
 " getMe: async () => pack(await A.getMe()),",
 ...
 " editMessage: async (p) => pack(await A.editMessage(p.msgId, p.text || p.body, p.options || {})),",
  " deleteMessage: async (p) => pack(await A.deleteMessage(p.msgId || p.id || p, p.for_everyone)),",
  " pinMessage: async (p) => pack(await A.pinMessage(p.msgId || p.id || p, p.pin, p.duration)),",
  " bulkDeleteMessages: async (p) => pack(await A.bulkDeleteMessages(p.msgIds || p.ids || p, p.for_everyone)),",
 ...
 " setAbout: async (p) => pack(await A.setStatusDOM(p.about || p.status)),",
  " getProfilePic: async (p) => pack(await A.getProfilePicUrl(p.chatId || p.value || p)),",
