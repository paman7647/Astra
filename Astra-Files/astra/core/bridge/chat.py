...
(function() {
 ...
 window.Astra.ensureWid = (value) => {
  ...
  if (options.mentions && Array.isArray(options.mentions)) {
    mentionedJidList = options.mentions.map(m => {
      const w = window.Astra.createWid(m);
      return w ? (w.userJid || w) : null;
    }).filter(Boolean);
  ...
  const message = {
   ...
   ...ephemeralFields,
   ...quotedMsgOptions,
   mentionedJidList: mentionedJidList.length > 0 ? mentionedJidList : undefined
  };
 ...
 window.Astra.pinChat = async (chatId, pin = true) => {
  ...
  window.Astra.pinMessage = async function(msgId, pin = true, duration = 86400) {
    try {
      const Store = window.Astra.initializeEngine();
      const msg = (typeof msgId === 'object') ? msgId : (Store.Msg.get(msgId) || (await Store.Msg.getMessagesById([msgId]))?.messages?.[0]);
      if (!msg) throw new Error("Message not found");

      console.log(`[Astra] pinMessage id=${msg.id._serialized} pin=${pin}`);

      const chat = Store.Chat.get(msg.id.remote) || await Store.Chat.find(msg.id.remote);
      
      // Strategy 1: Cmd module (Most reliable)
      if (Store.Cmd && (Store.Cmd.pinMsg || Store.Cmd.pinMessage)) {
        const func = Store.Cmd.pinMsg || Store.Cmd.pinMessage;
        await func(chat, msg, pin ? 1 : 0, duration);
        return true;
      }

      // Strategy 2: Direct model action
      if (msg.pin) {
        await msg.pin(pin ? 1 : 0, duration);
        return true;
      }

      // Strategy 3: Modern MEX/Pin module discovery
      const pinMod = window.Astra.mR.findModule(m => m && (typeof m.pinMsg === 'function' || typeof m.sendPinMsg === 'function'));
      if (pinMod) {
        const func = pinMod.pinMsg || pinMod.sendPinMsg;
        await func(chat, msg, pin ? 1 : 0, duration);
        return true;
      }

      throw new Error("Pinning module not found in this version of WhatsApp");
    } catch (e) {
      console.error('[Astra] pinMessage failed:', e.message);
      throw e;
    }
  };
