...
(function () {
  ...
  window.Astra.bridgeInProgress = false;
  window.Astra.withLock = async (fn) => {
    let timeout = 0;
    while (window.Astra.bridgeInProgress && timeout < 5000) {
      await new Promise(r => setTimeout(r, 100));
      timeout += 100;
    }
    window.Astra.bridgeInProgress = true;
    try {
      return await fn();
    } finally {
      window.Astra.bridgeInProgress = false;
  ...
  window.Astra.click = function(selector) {
  ...
  window.Astra.contextMenu = function(selector) {
    const el = document.querySelector(selector);
    if (el) {
  ...
  window.Astra.initializeEngine = function () {
    ...
    const moduleMap = {
      ...
      StatusAction: 'WAWebSendStatusMsgAction',
      ...
      MexGroupInvite: 'WAWebMexFetchGroupInviteCodeJob',
      GroupAction: 'WAWebModifyParticipantsGroupAction',
      GroupModify: 'WAWebGroupModifyInfoJob'
    };
    ...
    if (requireFunc) {
      ...
      const engineHeuristics = {
        ...
        FavoriteSticker: (m) => m && m.addFavoriteSticker && m.removeFavoriteSticker
      };
