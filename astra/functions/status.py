...
(function() {
 window.Astra = window.Astra || {};
  ...
  window.Astra.setMyStatus = async (args) => {
   const { status: text, options = {} } = args;
  ...
  let StatusV3Action = window.Store.StatusAction || window.Store.StatusUtils || window.Store.StatusV3Action;
  ...
  window.Astra.postStatus = async (args) => {
   const { data, mimetype: type, caption = "", options = {} } = args;
   return await window.Astra.withLock(async () => {
  const Store = window.Astra.initializeEngine();
